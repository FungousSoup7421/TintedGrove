...
public class ModRecipeProvider extends RecipeProvider implements IConditionBuilder {
    ...
    private void buildWoodSetRecipes(RecipeOutput recipeOutput, TintedColor color) {
        ...
        ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, ModBlocks.getPlank(color).get(), 4)
        ...
        stairBuilder(ModBlocks.getWoodStairs(color).get(), Ingredient.of(ModBlocks.getPlank(color)))
                .group(color.blockName("wood"))
                .unlockedBy("has_" + color.blockName("wood"), has(ModBlocks.getPlank(color)))
                .save(recipeOutput);

        slab(recipeOutput, RecipeCategory.BUILDING_BLOCKS, ModBlocks.getWoodSlab(color).get(), ModBlocks.getPlank(color).get());

        buttonBuilder(ModBlocks.getWoodButton(color).get(), Ingredient.of(ModBlocks.getPlank(color).get()))
                .group(color.blockName("wood"))
                .unlockedBy("has_" + color.blockName("wood"), has(ModBlocks.getPlank(color).get()))
                .save(recipeOutput);

        pressurePlate(recipeOutput, ModBlocks.getWoodPressurePlate(color).get(), ModBlocks.getPlank(color).get());

        fenceBuilder(ModBlocks.getWoodFence(color).get(), Ingredient.of(ModBlocks.getPlank(color).get()))
                .group(color.blockName("wood"))
                .unlockedBy("has_" + color.blockName("wood"), has(ModBlocks.getPlank(color).get()))
                .save(recipeOutput);

        fenceGateBuilder(ModBlocks.getWoodFenceGate(color).get(), Ingredient.of(ModBlocks.getPlank(color).get()))
                .group(color.blockName("wood"))
                .unlockedBy("has_" + color.blockName("wood"), has(ModBlocks.getPlank(color).get()))
                .save(recipeOutput);

        doorBuilder(ModBlocks.getWoodDoor(color).get(), Ingredient.of(ModBlocks.getPlank(color).get()))
                .group(color.blockName("wood"))
                .unlockedBy("has_" + color.blockName("wood"), has(ModBlocks.getPlank(color).get()))
                .save(recipeOutput);

        trapdoorBuilder(ModBlocks.getWoodTrapdoor(color).get(), Ingredient.of(ModBlocks.getPlank(color).get()))
                .group(color.blockName("wood"))
                .unlockedBy("has_" + color.blockName("wood"), has(ModBlocks.getPlank(color).get()))
                .save(recipeOutput);
    }
